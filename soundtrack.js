...
function updateSoundtrack(newVideoId, shouldPlay = true) {
    ...
    // 2. Verificamos si el reproductor de YouTube está listo y sano
    if (player && isPlayerReady && typeof player.getIframe === 'function' && player.getIframe()) {
        ...
        if (shouldPlay) {
            ...
            // Actualiza el icono del botón a "Sonando"
            ...
            // Actualiza el icono del botón a "Mudo/Pausado"
            if (typeof actualizarVisualesMusica === 'function') actualizarVisualesMusica(false);
        }

    } else {
        // 3. Si el player falló o no existe, lo reiniciamos por completo
...
function onPlayerStateChange(event) {
    // --- 1. TU LóGICA DE SINCRONIZACIóN (Intacta) ---
    ...
    // --- 2. ACTUALIZACIóN DE ESTADOS (Para que el botón lata o no) ---
...
setInterval(() => {
    if (isMusicPlaying && player && isPlayerReady) {
        ...
        // Si debería sonar pero está en pausa (2) o buffering eterno (3)
